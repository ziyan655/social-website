...
<head>
	...
	<title>Artist Post Concert</title>
	<link href="../css/bootstrap.min.css" rel="stylesheet"></link>
	<link rel="stylesheet" href="../css/apost.css"></link>
...
if(!empty($_POST['band'])){
	...
	if($stmt->fetch()) {
		$stmt->close();
		$stmt = $mysqli->prepare("select bandId from Artist where bandId = ? AND artistId = ?");
		$stmt->bind_param("ii", $bandid, $_SESSION['artistid']);
...
if($insertCname && $insertDate && $insertLoc && $insertCity && $insertCountry && $insertTime && $insertBand) {
	$cname=$_POST["cname"];
	...
	if(empty($_POST["band"])) {
		...
		$stmt->bind_param("ii", $_SESSION["artistid"],$cid); 
		$stmt->execute();
		$stmt->close();
	}
}
